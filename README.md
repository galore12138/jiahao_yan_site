# Jiahao YAN 鈥?Academic Website

Personal academic website for Jiahao YAN, built with Hugo and designed for GitHub Pages.

SEO basics are included for discoverability: canonical URLs, meta description, Open Graph/Twitter preview metadata, a robots.txt file, Hugo's sitemap, and Schema.org Person structured data for search engines.

## Local preview

From the repository root:

```powershell
.\.tools\hugo\hugo.exe server --disableFastRender
```

Then open <http://localhost:1313/jiahao_yan_site/>.

The site has not been published merely by creating these files. Publication occurs only after the repository is pushed and GitHub Pages is enabled.

